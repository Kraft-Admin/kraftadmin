    ...
    init {
        try {
            connection.createStatement().use { statement ->
                ...
                // 5. Outbound HTTP Client Requests Table
                statement.execute("""
...
""")
                ...
                statement.execute("CREATE INDEX IF NOT EXISTS idx_http_client_created ON kraft_http_client_events(created_at)")
                ...
                println("✅ KraftPulse: SQLite Schema Synchronized.")
            }
        } catch (e: Exception) {
    ...
    // --- FETCH METHODS ---
        ...
        val sql = "SELECT payload FROM kraft_telemetry ORDER BY created_at ASC LIMIT ?"
        ...
        try {
            connection.prepareStatement(sql).use { pstmt ->
                ...
                while (rs.next()) {
                    ...
                    events.add(serializer.fromJson(json, KraftTelemetryEvent::class.java))  // ← interface call
                }
            }
        } catch (e: Exception) {
    ...
    /**
     * The Laravel Nightwatch "Deep Dive": Unified view of everything in one trace.
     */
    ...
    // DELETE
        ...
        val sql = "DELETE FROM kraft_telemetry WHERE id IN ($placeholders)"
        try {
            connection.prepareStatement(sql).use { pstmt ->
                ids.forEachIndexed { index, id -> pstmt.setString(index + 1, id) }
                pstmt.executeUpdate()
            }
        } catch (e: Exception) {
            System.err.println("KraftAdmin SQLite Error (DeleteBatch): ${e.message}")
        }
    }
    ...
    fun pruneOldEvents(retentionDays: Int = 7) {
        ...
        listOf(
            ...
            connection.prepareStatement("DELETE FROM $table WHERE created_at < ?").use { pstmt ->
    ...
    // Generic fetch utility to power dedicated list view pages
    ...
    /**
     * ENHANCED DEEP DIVE: Captures all operational relations for a
     * completely stacked timeline layout on your Front-end UI.
     */
