...
//
//    fun lookup(lookup: LookupDescriptor, searchQuery: String?, limit: Int = 20): List<ObjectResponse> {
//        val entityClass = resolveEntityClass(lookup.targetEntity ?: return emptyList()) ?: return emptyList()
//
...
//                .firstOrNull { it.name.equals(entityName, ignoreCase = true) }
//                ?.javaType
        ...
        return try {
            entityManager.entityManagerFactory.metamodel.entities
                .firstOrNull { entityType ->
                    // ✅ Match JPA entity name (@Entity(name=...))
                    entityType.name.equals(entityName, ignoreCase = true) ||
                            // ✅ Match Java class simple name (ProductCategory)
