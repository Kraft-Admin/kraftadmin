 ...
 *     @KraftAdminOn(KraftAdminEvent.AfterCreate::class, resource = "Order")
 *     fun onOrderCreated(event: KraftAdminEvent.AfterCreate) {
 *         emailService.sendConfirmation(event.entity as Order)
 *     }
 *
 *     // Veto example — fires before delete, throws to cancel
 *     @KraftAdminOn(KraftAdminEvent.BeforeDelete::class, resource = "Order")
 *     fun preventDeleteIfShipped(event: KraftAdminEvent.BeforeDelete) {
 ...
 *     @KraftAdminOn(KraftAdminEvent.AfterDelete::class, async = true, order = 100)
 *     fun auditDelete(event: KraftAdminEvent.AfterDelete) {
...
annotation class KraftAdminOn(
    ...
    /**
     * Filter by resource name string — matches event.resourceName.
     * Blank = all resources.
